...
export const Route = createFileRoute("/login")({
  ...
  component: LoginPage,
  head: () => ({ meta: [{ title: "Masuk — ATHA AI Study Hub" }] }),
});
...
function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    if (result.error) { setLoading(false); return toast.error("Login Google gagal. Coba lagi."); }
  ...
  return (
    <div className="min-h-screen grid place-items-center px-4 py-12 relative overflow-hidden">
      <div className="glow-orb w-[500px] h-[500px] -top-40 -right-40 animate-pulse-glow" />
      <motion.div
      ...
      >
        ...
        <h1 className="text-2xl font-bold tracking-tight text-center mb-2">Selamat datang kembali</h1>
        <p className="text-sm text-muted-foreground text-center mb-8">Masuk untuk lanjut belajar dengan AI</p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-1.5">
              ...
              <Input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="pl-9 h-11" />
            </div>
          </div>
